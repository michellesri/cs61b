...
public class MapServer {
    ...
    static class TrieNode {
        ...
        boolean exists = false; // if it exists, it means the current node ends a word
    ...
    public static void initialize() {
        ...
        trie = new TrieNode();
        for (long nodeId : graph.vertices()) {
            String name = graph.getGraphName(nodeId);

        }
    }

    private void addTrieNode(TrieNode root, String name) {
        for (char c : name.toCharArray()) {
            //TODO: add word to trie
                // check if word exists
                // if word does not exist add
        }
    }
